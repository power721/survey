export default {
    common: {
        appName: 'アンケート＆投票',
        home: 'ホーム',
        login: 'ログイン',
        register: '新規登録',
        logout: 'ログアウト',
        submit: '送信',
        save: '保存',
        cancel: 'キャンセル',
        delete: '削除',
        edit: '編集',
        create: '作成',
        search: '検索',
        reset: 'リセット',
        back: '戻る',
        confirm: '確認',
        loading: '読み込み中...',
        noData: 'データがありません',
        actions: '操作',
        status: 'ステータス',
        published: '公開済み',
        draft: '下書き',
        closed: '終了',
        public: '公開',
        private: '非公開',
        yes: 'はい',
        no: 'いいえ',
        required: '必須',
        optional: '任意',
        darkMode: 'ダークモード',
        language: '言語',
        profile: 'プロフィール',
        editProfile: 'プロフィール編集',
        admin: '管理者',
        oldPassword: '現在のパスワード',
        newPassword: '新しいパスワード',
        changePassword: 'パスワード変更',
        saveSuccess: '保存しました',
        creator: '作成者',
        createdAt: '作成日時',
        startTime: '開始時刻',
        time: '時刻',
        startTimeRequired: '開始時刻を選択してください',
        endTimeBeforeStartTime: '終了日時は開始日時より後に設定してください',
        logoUrl: 'ロゴ画像URL',
        logoUrlPlaceholder: 'ロゴ画像のURLを入力（任意）',
        backgroundUrl: '背景画像URL',
        backgroundUrlPlaceholder: '背景画像のURLを入力（任意）',
    },
    auth: {
        username: 'ユーザー名',
        password: 'パスワード',
        email: 'メールアドレス',
        nickname: 'ニックネーム',
        loginTitle: 'ログイン',
        registerTitle: '新規登録',
        noAccount: 'アカウントをお持ちでない方',
        hasAccount: 'アカウントをお持ちの方',
        goRegister: '新規登録へ',
        goLogin: 'ログインへ',
        loginSuccess: 'ログインしました',
        registerSuccess: '登録が完了しました',
        orLoginWith: '他の方法でログイン',
        loginWithGithub: 'GitHubでログイン',
        loginWithGoogle: 'Googleでログイン',
        oauth2Failed: 'ソーシャルログインに失敗しました',
    },
    survey: {
        title: 'アンケート',
        mySurveys: 'マイアンケート',
        publicSurveys: '公開アンケート',
        templates: 'テンプレート',
        createSurvey: 'アンケート作成',
        editSurvey: 'アンケート編集',
        surveyTitle: 'アンケートタイトル',
        description: '説明',
        questions: '質問',
        addQuestion: '質問を追加',
        copyQuestion: '質問をコピー',
        questionTitle: '質問タイトル',
        questionType: '質問タイプ',
        options: '選択肢',
        addOption: '選択肢を追加',
        batchAddOptions: '一括追加',
        batchAddOptionsHint: '1行に1つの選択肢を入力してください。空行はスキップされます',
        batchAddOptionsPlaceholder: '選択肢1\n選択肢2\n選択肢3',
        addedOptions: '{count}個の選択肢を追加しました',
        publish: '公開',
        close: '終了',
        fill: 'アンケートに回答',
        responses: '回答一覧',
        statistics: '統計分析',
        export: 'Excel出力',
        qrCode: 'QRコード',
        download: 'ダウンロード',
        anonymous: '匿名',
        accessLevel: 'アクセス権限',
        endTime: '締め切り',
        responseCount: '回答数',
        notStarted: 'アンケートはまだ開始されていません',
        endTimeRequired: '締め切りを設定してください',
        atLeastOneQuestion: '少なくとも1つの質問が必要です',
        section: 'セクション',
        addSection: 'セクションを追加',
        prevSection: '前へ',
        nextSection: '次へ',
        submitSuccess: 'アンケートを送信しました',
        thankYou: 'ご参加ありがとうございます！',
        uploadFile: 'ファイルアップロード',
        loginRequired: '非匿名アンケートにはログインが必要です',
        alreadySubmitted: '送信済み',
        viewMyResponse: 'このアンケートは既に回答済みです。以下があなたの回答内容です。',
        allowUpdate: '回答の更新を許可',
        updateResponse: '回答を更新',
        maxResponses: '最大回答数',
        maxResponsesPlaceholder: '未入力の場合は無制限',
        quotaReached: 'このアンケートは最大回答数に達しました',
        conditionLogic: '条件分岐',
        conditionQuestion: '設問',
        conditionOption: 'の回答が',
        conditionThen: 'のとき表示',
        conditionNone: '条件なし（常に表示）',
        conditionRemove: '条件を削除',
        advancedSettings: '詳細設定',
        optionLimits: '選択数制限',
        valueLimits: '値の制限',
        minOptions: '最小選択数',
        maxOptions: '最大選択数',
        minOptionsPlaceholder: '空白で制限なし',
        maxOptionsPlaceholder: '空白で制限なし',
        minValue: '最小値',
        maxValue: '最大値',
        minValuePlaceholder: '空白で制限なし',
        maxValuePlaceholder: '空白で制限なし',
        types: {
            SINGLE_CHOICE: '単一選択',
            MULTIPLE_CHOICE: '複数選択',
            TEXT: '短文',
            TEXTAREA: '長文',
            NUMBER: '数値',
            RATING: '評価',
            DATE: '日付',
            EMAIL: 'メール',
            URL: 'URL',
            PHONE: '電話番号',
            ID_CARD: 'ID番号',
            FILE: '添付ファイル',
        },
    },
    vote: {
        title: '投票',
        myVotes: 'マイ投票',
        publicVotes: '公開投票',
        createVote: '投票作成',
        editVote: '投票編集',
        voteTitle: '投票タイトル',
        description: '投票の説明',
        voteType: '投票タイプ',
        single: '単一選択',
        multiple: '複数選択',
        scored: 'スコア投票',
        frequency: '投票頻度',
        once: '1回のみ',
        daily: '1日1回',
        maxTotalVotes: '1人あたりの最大投票数',
        maxVotesPerOption: '1項目あたりの最大投票数',
        minOptions: '最小選択数',
        minOptionsPlaceholder: '未入力の場合は無制限',
        maxOptions: '最大選択数',
        remainingVotes: '残り投票数',
        optionTitle: '選択肢タイトル',
        optionContent: '選択肢の説明',
        imageUrl: '画像URL',
        publish: '公開',
        close: '終了',
        submitVote: '投票する',
        results: '投票結果',
        votes: '票数',
        hasVoted: '既に投票済みです',
        voteSuccess: '投票しました',
        realtime: 'リアルタイム結果',
        endTime: '締め切り',
        totalVotes: '総投票数',
        showVoters: '投票者を表示',
        endTimeRequired: '締め切りを設定してください',
        atLeastOneOption: '少なくとも1つの選択肢が必要です',
        atLeastTwoOptions: '少なくとも2つの選択肢が必要です',
        maxTotalVotesRequired: '1人あたりの最大投票数を設定してください',
        maxVotesPerOptionRequired: '1項目あたりの最大投票数を設定してください',
        expired: '締め切り済み',
        loginRequired: '非匿名投票にはログインが必要です',
        votingClosed: '投票は締め切られました',
        records: '投票記録',
        voter: '投票者',
        votedOption: '投票した項目',
        votedAt: '投票日時',
        anonymous: '匿名',
        notStarted: '投票はまだ開始されていません',
        myVoteHistory: '投票履歴',
        voted: '投票済み',
        selectedOption: '選択した項目',
        viewPollDetails: '投票詳細を表示',
        noVoteHistory: '投票履歴がありません',
        startCountdown: '開始まで',
        endCountdown: '終了まで',
        days: '日 ',
        hours: '時間 ',
        minutes: '分 ',
        seconds: '秒',
        batchAdd: '一括追加',
        batchAddPlaceholder: '1行に1つの選択肢を入力\n末尾にスペースを空けて画像URLを追加可能（任意）\n\n例：\n選択肢A\n選択肢B https://example.com/image.png',
        batchAddEmpty: '少なくとも1つの選択肢を入力してください',
        batchAddSuccess: '{count} 件の選択肢を追加しました',
    },
    admin: {
        systemConfig: 'システム設定',
        basicSettings: '基本設定',
        siteTitle: 'サイトタイトル',
        siteTitlePlaceholder: 'サイトタイトルを入力',
        siteDescription: 'サイト説明',
        siteDescriptionPlaceholder: 'サイトの説明を入力',
        siteLogo: 'サイトロゴ',
        siteFooter: 'フッターテキスト',
        siteFooterPlaceholder: '例: © 2025 会社名. All rights reserved.',
        timezone: 'タイムゾーン',
        userSettings: 'ユーザーと登録',
        registerEnabled: '新規登録を許可',
        oauth2Settings: 'ソーシャルログイン',
        oauth2Enabled: 'ソーシャルログインを有効にする',
        oauth2RedirectUri: 'コールバックURL (Redirect URI)',
        clientId: 'Client ID',
        clientSecret: 'Client Secret',
        uploadSettings: 'ファイルアップロード',
        uploadMaxSize: '最大ファイルサイズ',
        uploadAllowedExtensions: '許可する拡張子',
        uploadAllowedExtensionsPlaceholder: '.jpg,.png,.pdf,...',
        securitySettings: 'セキュリティ',
        loginMaxAttempts: '1分あたりの最大ログイン試行回数',
        jwtExpiration: 'セッション有効期間',
        hours: '時間',
    },
    dashboard: {
        title: 'ダッシュボード',
        totalUsers: '総ユーザー数',
        totalSurveys: '総アンケート数',
        totalResponses: '総回答数',
        totalVotes: '総投票数',
        totalVoteRecords: '総投票記録数',
        trends: 'データトレンド',
        userTrend: 'ユーザー増加（直近7日間）',
        surveyTrend: 'アンケート作成（直近7日間）',
        responseTrend: 'アンケート回答（直近7日間）',
    },
    editor: {
        bold: '太字',
        italic: '斜体',
        underline: '下線',
        strikethrough: '取り消し線',
        bulletList: '箇条書き',
        orderedList: '番号付きリスト',
        link: 'リンク',
        linkPlaceholder: 'URLを入力',
        image: '画像',
        imagePlaceholder: '画像URLを入力',
        paragraph: '本文',
        heading1: '見出し 1',
        heading2: '見出し 2',
        heading3: '見出し 3',
        superscript: '上付き文字',
        subscript: '下付き文字',
        clearFormat: '書式をクリア',
    },
    home: {
        description: 'アンケートや投票を簡単に作成し、データを収集・分析できます',
        surveyFeatures: '単一選択、複数選択、テキスト、数値、評価、日付など多様な質問形式に対応',
        voteFeatures: '単一/複数選択投票、不正防止機能、リアルタイム結果表示に対応',
    },
    nav: {
        home: 'ホーム',
        surveys: 'アンケート',
        votes: '投票',
        manage: '管理',
    },
    audit: {
        title: '操作監査',
        username: 'ユーザー名',
        action: '操作',
        entityType: 'エンティティタイプ',
        entityId: 'エンティティID',
        details: '詳細',
        ipAddress: 'IPアドレス',
        createdAt: '操作時刻',
        entityTypes: {
            Survey: 'アンケート',
            Vote: '投票',
            User: 'ユーザー',
        },
        actions: {
            CREATE_SURVEY: 'アンケート作成',
            UPDATE_SURVEY: 'アンケート更新',
            DELETE_SURVEY: 'アンケート削除',
            EXPORT_SURVEY: 'アンケートエクスポート',
            CREATE_VOTE: '投票作成',
            UPDATE_VOTE: '投票更新',
            DELETE_VOTE: '投票削除',
            USER_LOGIN: 'ユーザーログイン',
            USER_REGISTERED: 'ユーザー登録',
            LOGIN_FAILED: 'ログイン失敗',
            REGISTER_FAILED: '登録失敗',
            USER_PROFILE_UPDATED: 'プロフィール更新',
        },
    },
}
