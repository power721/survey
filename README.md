...
- 自定义**排序顺序**（拖拽排序）
- 添加**题目描述**
- **多选题约束**：可设置最少/最多选项数（高级设置）
- **数字题约束**：可设置最小/最大值（高级设置）
- **条件逻辑**：根据前置题目的答案决定是否显示（高级设置）
- **高级设置**：默认折叠，点击展开显示约束条件和跳题逻辑
...
- **题目约束验证**：
  - 多选题：验证选择数量在设置的最小/最大范围内
  - 数字题：验证输入值在设置的最小/最大范围内
...
| 功能       | 说明                                         | 状态 |
|----------|--------------------------------------------|----|
| **操作记录** | 记录关键操作：创建/更新/删除问卷、创建/更新/删除投票、导出数据、用户登录/注册/更新信息、登录/注册失败等 | ✅  |
| **审计信息** | 包含操作时间、操作用户、操作类型、实体类型、实体 ID、IP 地址、详细信息 | ✅  |
| **筛选查询** | 支持按操作类型、实体类型、用户名筛选，分页浏览所有审计日志              | ✅  |
| **权限控制** | 仅管理员可访问                                    | ✅  |
